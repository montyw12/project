...
function createOrder($distributorId, $itemSelected, $itemQuantity)
{
        ...
        if (isset($item)) {
            ...
            if ($data1["amount"] >= $totalOrderAmount) {
                ...
                if (mysqli_stmt_prepare($stmt2, $queryString2_1)) {
                    mysqli_stmt_bind_param($stmt2, "ds", $totalOrderAmount, $producerId);
                    mysqli_stmt_execute($stmt2);
                }
                ...
                if (mysqli_stmt_prepare($stmt3, $queryString3)) {
                    mysqli_stmt_bind_param($stmt3, "ss", $producerId, $distributorId);
    ...
    unset($totalOrderAmount, $key, $value, $producerId, $a, $item, $dbConn, $queryString, $stmt, $index, $item_id, $result, $data, $queryString1, $stmt1, $result1, $data1, $queryString2, $queryString2_1, $stmt2, $queryString3, $stmt3, $result3, $data3, $f_provider_client_id, $queryString4, $order_id, $item_no, $order_date, $status, $stmt4, $queryString5, $stmt5, $distributorId, $itemSelected, $itemQuantity);
    return $resultToReturn;
}
...
function errorsForCreateOrder($error_code)
{
    switch ($error_code) {
        ...
        case 1:
            ...
            break;
        case 2:
            $qs = "error=" . base64_encode("Provide sufficient information to make this order");
            header("location: ./orders_make.php?" . $qs);
            ob_end_clean();
            exit();
            break;
