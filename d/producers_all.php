...
<?php
try {
    require_once("./php/producers_all.fun.php");

    $result = selectAllProducers($_SESSION["user_id"]);
    ...
    <div class="row">
                    ...
                    <div class="card-footer">
                        <form method="post">
                            ...
                            <?php elseif ($data["status"] == 1 || $data["status"] == 2) : ?>
                                <button class="w3-button w3-yellow w3-round-large w3-hover-yellow" name="Pending" value="<?= $data["user_id"] ?>" disabled>Pending</button>
        ...
        <?php if ($whileIteration == 0) : ?>
            <div class="col-12 mt-5 text-muted" style="text-align: center;">
                <h5>Producers all section is empty!</h5>
            </div>
        <?php endif; ?>
    </div>
</div>
