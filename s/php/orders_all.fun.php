...
{
    $queryString = "SELECT order_id, f_provider_id, order_date, dispatch_date, delivery_date, orders.status, item_no, r_id FROM orders LEFT JOIN provider_client ON f_provider_client_id = r_id WHERE f_client_id = ? AND provider_client.status = 3 AND orders.status != 0 ORDER BY orders.status, order_date;";
