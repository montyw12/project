...
{
    $queryString = "SELECT user_id, type, name, address, email, item_has, status FROM users LEFT JOIN provider_client ON user_id = f_provider_id WHERE type = 'distributor' AND f_client_id = ? AND status = 3 ORDER BY name;";
...
// #3 function
function disconnectDistributor($sellerId, $distributorId)
{
    ...
    mysqli_stmt_close($stmt);
    databaseConnectorClose($dbConn);
    unset($queryString, $dbConn, $stmt, $sellerId, $distributorId);
    return $resultToReturn;
}
