...
function selectAllDistributors(){
    $queryString = "SELECT user_id, type, name, address, email, item_has, status FROM user LEFT JOIN provider_client ON user_id = f_client_id WHERE type = 'distributor' ORDER BY name;";
...
function requestForConnect($producer_id, $distributor_id){
    $queryString = "INSERT INTO provider_client(f_provider_id, f_client_id) VALUE(?, ?);";
    $dbConn = databaseConnector();
    $stmt = mysqli_stmt_init($dbConn);
    if (mysqli_stmt_prepare($stmt, $queryString)) {
